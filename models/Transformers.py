...
class SCCLBert(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, task_type="virtual"):
        if task_type == "evaluate":
            ...
            mean_output_1 = F.normalize(mean_output_1, p=2, dim=1)*10
            mean_output_2 = self.get_mean_embeddings(input_ids_2, attention_mask_2)
            mean_output_2 = F.normalize(mean_output_2, p=2, dim=1)*10
    ...
    def get_cluster_prob(self, embeddings, centers):
        norm_squared = torch.sum((embeddings.unsqueeze(1) - torch.tensor(centers).cuda()) ** 2, 2)
